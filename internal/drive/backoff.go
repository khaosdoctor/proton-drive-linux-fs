...
// usable Retry-After hint (see retryAfterHint).
...
// rateLimit is the shared "stop calling for a while" window every API call site checks before
// issuing a new request (see RateLimited):
...
// ErrRateLimited signals that Proton's rate-limit window is still active; fusefs maps it to
// ETIMEDOUT.
...
// before giving up. Called once from fusefs.Mount with Options.MetaTimeout.
func (c *Client) SetMetaTimeout(d time.Duration) {
	c.metaTimeout = d
}

// noteRateLimitStatus opens or extends the shared backoff window for a 429/503 response.
...
// IsUnrecoverableAuth reports whether err is a 422/Code-10013 "refresh token revoked"
// response. Bare 401s are not matched: go-proton-api handles those internally via its
// refresh mechanism. When true the only fix is re-reading session.json by restarting.
